...
def start_maj   #Combien commencent par une majuscule (première lettre juste après le @) ?
    count = 0
    journalists_tab = array
    journalists_tab.each do |i|
      if i[1] =~ /[A-Z]/
...
def handle_position
  journalists_tab = array
   puts "La position dans l'index de @epenser est #{journalists_tab.find_index("@epenser")}."
end
